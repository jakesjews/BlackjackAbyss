...
import {
  REWARD_BUTTON_STYLE,
  REWARD_MODAL_CLOSE_DEPTH,
  REWARD_MODAL_CONTENT_DEPTH,
  REWARD_PRIMARY_GOLD,
...
function ensureRewardSceneLogsCloseButton(scene, onPress) {
...
export function drawRewardSceneLogsModal(scene, { snapshot, width, height }) {
  ...
  const title = scene.drawText("reward-logs-title", "RUN LOGS", x + 18, y + 26, {
  ...
  visible.forEach((line, index) => {
    ...
    const row = scene.drawText(`reward-logs-line-${index}`, displayLine, listX + 12, rowY + rowH * 0.5, {
  ...
  const waiting = scene.drawText("reward-logs-waiting", `waiting${dots}`, listX + listW - 2, y + modalH - 14, {
    fontFamily: '"Sora", "Segoe UI", sans-serif',
  ...
  const closeButton = ensureRewardSceneLogsCloseButton(scene, () => {
