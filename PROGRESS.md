...
- Last Updated: 2026-02-22 21:55:00 EST
...
- Extracted RunScene modal state/order/blocker helpers into `src/engine/scenes/run/run-scene-modals.js` and removed duplicated modal state methods from `RunScene`.
...
- Shifted scene mode sync ownership to app runtime context (`runtime.reportMode`).
...
- Tightened acceptance boot contracts around runtime API contracts and hooks.
- Removed bridge API write-registration flow from runtime API setup; runtime APIs are canonical.
...
- Removed `window.__ABYSS_PHASER_BRIDGE__` publication and deleted `src/engine/runtime/compat/phaser-bridge-compat.js`.
- Renamed scene/runtime helper to `src/engine/scenes/runtime-access.js` and core runtime wiring modules to runtime-native names (`phaser-runtime-apis`, `runtime-mode-sync`).
...
- Extracted Phaser runtime API registration blocks into `src/engine/runtime/core/phaser-runtime-apis.js` and reduced bootstrap menu/run/reward/shop/overlay API registration functions to delegation wrappers with dedicated unit tests.
...
- Removed `src/engine/legacy/legacy-runtime-adapter.js` and rewired `src/engine/app.js` + `src/engine/scenes/runtime-access.js` to a direct runtime context/tick seam.
...
- Extracted runtime mode-property bridge installation into `src/engine/runtime/core/runtime-mode-sync.js` and removed the inline mode-bridge block from `src/engine/runtime/runtime-engine.js`.
- Added targeted tests for mode bridging in `src/engine/runtime/__tests__/runtime-mode-sync.test.mjs`.
...
- `npm run test:smoke`: failing on current visual threshold miss (`desktop-1280x720/03b-playing-actions`, `diffPixels=298`, `diffRatio=0.00032335069444444447`, `maxDiffPixels=250`).
