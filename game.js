(() => {
  ...
  function drawEncounter() {
    ...
    ctx.fillStyle = "#d7e8f8";
    setFont(17, 700, false);
    ctx.fillText(`Hand ${playerTotal}`, playerBox.centerX, Math.min(HEIGHT - 8, playerBarY + barH + handLabelGap));
  }
  ...
  function drawShopScreen() {
    ...
    renderCards.forEach((card) => {
      ...
      if (card.selected) {
        setFont(18, 600, false);
        wrapText(shopItemDescription(item), card.x + 32, card.y + 112, card.w - 64, 22, "center");
      } else {
        setFont(15, 600, false);
        ctx.fillText(item.sold ? "Sold out" : "", card.x + card.w * 0.5, card.y + card.h - 86);
      }
      ...
      const btnW = Math.min(card.w - 28, card.selected ? 188 : 164);
      const btnH = card.selected ? 36 : 32;
      const btnX = card.x + (card.w - btnW) * 0.5;
      const btnY = card.y + card.h - btnH - 20;
      ...
      setFont(card.selected ? 18 : 16, 700, false);
